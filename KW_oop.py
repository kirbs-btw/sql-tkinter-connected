...
class main():
    ...
    def printEmployees(self, company, frame):
        """
        ...
        obj.printTable(frame, employeeTable)

        conn.close()

        pass

    def printTable(self, frame, table):
        """
        ...
        for i in table:
            ...
            tk.Button(frame, text="pick", justify="left", command=lambda m=i[4]: obj.printID(m)).grid(row=f, column=5,
                                                                                                  pady=10, padx=10,
                                                                                                  sticky="w")
    ...
    def printTableProjects(self, frame, root):
        conn = sqlite3.connect("kw.sql")
        cur = conn.cursor()
        ...
        for i in table:
            ...
            tk.Button(frame, text="Choose Project", justify="left", command=lambda m=i[0]: obj.chooseProject(m, root)).grid(
                row=f, column=5, pady=10, padx=10, sticky="w")

    def chooseProject(self, projectId, root):
        """
    ...
    def printProjectReq(self, canvas):
        conn = sqlite3.connect("kw.sql")
        cur = conn.cursor()
    ...
    def openProjectsWindow(self, root):
        """
        ...
        obj.printTableProjects(frameScroll, root)
    ...
    def mainWindow(self):
        ...
        kwButton = tk.Button(canvas, image=kw, borderwidth=0, command=lambda: obj.passwordDev(), bg='#ffffff')
        ...
        refreshButton = tk.Button(canvas, image=refreshImg, borderwidth=0, bg='#ffffff',
                                  command=lambda: obj.printEmployees(selectedCompany.get(), frameScroll))
        ...
        projectsWindowButton = tk.Button(canvas, command=lambda: obj.openProjectsWindow(root), text="Choose project")
        ...
        obj.printEmployees(selectedCompany.get(), frameScroll)
        obj.printProjectReq(frameScrollReq)

        root.mainloop()

    def login(self, entry, root):
        """
    ...
    def passwordDev(self):
        ...
        loginButton = tk.Button(canvas, text="login", command=lambda: obj.login(passwordInput.get(), root))
    ...
    def devWindow(self):
        ...
        executeButton = tk.Button(canvas, borderwidth=0, text="okookokokokokokok", bg='#ffffff',
                                  command=lambda: obj.executeSqlCommand(commandInput.get()))
